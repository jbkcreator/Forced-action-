"""Apply CDE-04/CDE-06 go-live — add tax_deed_outcomes + appraiser_sale_outcomes
to ScraperRunStats.check_run_stats_source_type.

Deferred from Wave 0 by design ("each wave-1 connector adds its own
source_type at go-live, not before" — src/connectors/registry.py). Both
connectors are now built and enabled; this unblocks record_scraper_stats()
for them (it fails soft today — non-critical, logged — but the run stats
were silently not being recorded).

Idempotent — DROP CONSTRAINT IF EXISTS before re-adding.

Usage:
    PYTHONPATH=. python migrations/apply_cde04_cde06_scraper_run_stats_source_types.py
"""
...
def main() -> None:
    ...
    logger.info("cde04_cde06_scraper_run_stats_source_types complete.")
