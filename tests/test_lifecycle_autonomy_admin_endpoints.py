"""
fa036 — HTTP-layer integration tests for the Lifecycle autonomy admin endpoints.

Covers the 4 endpoints added to src/api/admin_router.py:
  POST /api/admin/lifecycle-playbook/{id}/adopt
  POST /api/admin/lifecycle-playbook/{id}/reject
  POST /api/admin/lifecycle-playbook/{id}/retire
  GET  /api/admin/lifecycle-autonomy
...
class TestAuthRequired:
    ...
    def test_adopt_requires_auth(self, client):
        r = client.post("/api/admin/lifecycle-playbook/1/adopt", json={"actor": "dev"})
        assert r.status_code in (401, 403)

    def test_reject_requires_auth(self, client):
        r = client.post("/api/admin/lifecycle-playbook/1/reject", json={"actor": "dev"})
        assert r.status_code in (401, 403)

    def test_retire_requires_auth(self, client):
        r = client.post("/api/admin/lifecycle-playbook/1/retire", json={"actor": "dev"})
        assert r.status_code in (401, 403)

    def test_lifecycle_autonomy_get_requires_auth(self, client):
        r = client.get("/api/admin/lifecycle-autonomy")
        assert r.status_code in (401, 403)
...
class TestAdoptEndpoint:
    ...
    def test_happy_path_returns_200_and_adopted(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)), \
            ...
            r = client.post(
                "/api/admin/lifecycle-playbook/42/adopt",
    ...
    def test_404_when_playbook_missing(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)), \
             patch("src.services.playbook_writer.transition_status", return_value=False):
            r = client.post(
                "/api/admin/lifecycle-playbook/9999/adopt",
                headers=auth_headers,
                json={"actor": "dev@heu.ai"},
            )
...
class TestRejectEndpoint:
    ...
    def test_happy_path_returns_200_with_reason(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)), \
             patch("src.services.playbook_writer.transition_status", return_value=True) as mock_t:
            r = client.post(
                "/api/admin/lifecycle-playbook/42/reject",
                headers=auth_headers,
    ...
    def test_404_when_playbook_missing(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)), \
             patch("src.services.playbook_writer.transition_status", return_value=False):
            r = client.post(
                "/api/admin/lifecycle-playbook/9999/reject",
                headers=auth_headers,
                json={"actor": "dev@heu.ai"},
            )
        assert r.status_code == 404
...
class TestRetireEndpoint:
    ...
    def test_happy_path_returns_200(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)), \
             patch("src.services.playbook_writer.transition_status", return_value=True) as mock_t:
            r = client.post(
                "/api/admin/lifecycle-playbook/42/retire",
                headers=auth_headers,
                json={"actor": "dev@heu.ai"},
            )
        assert r.status_code == 200
    ...
    def test_404_when_playbook_missing(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)), \
             patch("src.services.playbook_writer.transition_status", return_value=False):
            r = client.post(
                "/api/admin/lifecycle-playbook/9999/retire",
                headers=auth_headers,
                json={"actor": "dev@heu.ai"},
            )
        assert r.status_code == 404
...
# ---------------------------------------------------------------------------
# GET /lifecycle-autonomy
# ---------------------------------------------------------------------------

class TestLifecycleAutonomyGet:
    ...
    def _session_with_cards(self, n_cards):
        ...
        for i in range(n_cards):
            rows.append(SimpleNamespace(
                ...
                summary_text=f"Lifecycle autonomy: {80 - i}% autonomous, n/a overridden, 2 adopted, +1 net playbooks",
                data_json={
                    "autonomous_pct": 80 - i,
                    "overridden_pct": None,
    ...
    def test_returns_latest_plus_history(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)):
            r = client.get("/api/admin/lifecycle-autonomy", headers=auth_headers)
        assert r.status_code == 200
        body = r.json()
        assert body["ok"] is True
    ...
    def test_weeks_query_param_respects_range(self, client, auth_headers):
        ...
        with patch("src.api.admin_router.get_db_context", _fake_db_ctx(sess)):
            r = client.get("/api/admin/lifecycle-autonomy?weeks=4", headers=auth_headers)
        assert r.status_code == 200
    ...
    def test_weeks_param_validated_to_max_52(self, client, auth_headers):
        r = client.get("/api/admin/lifecycle-autonomy?weeks=999", headers=auth_headers)
        assert r.status_code == 422

    def test_weeks_param_validated_min_1(self, client, auth_headers):
        r = client.get("/api/admin/lifecycle-autonomy?weeks=0", headers=auth_headers)
        assert r.status_code == 422
