...
def test_clean_run_is_healthy():
    assert sunbiz_run_verdict({"processed": 200, "failed": 0}) == (True, None, None, None)


def test_one_off_failure_is_still_healthy():
    # The exact case that false-alarmed daily under `failed == 0`.
    ok, etype, _, outcome = sunbiz_run_verdict({"processed": 200, "failed": 1})
    assert ok is True and etype is None and outcome is None


def test_below_rate_threshold_is_healthy():
    # 3 failures but only 1.5% — under the 15% rate gate → healthy.
    ok, etype, _, outcome = sunbiz_run_verdict({"processed": 200, "failed": 3})
    assert ok is True and etype is None and outcome is None


def test_frequent_failures_are_broken():
    ok, etype, msg, outcome = sunbiz_run_verdict({"processed": 200, "failed": 40})
    assert ok is False
    assert etype == "scraper_error"
    assert outcome == ScraperOutcome.SOURCE_ERROR.value
    assert "20%" in msg


def test_rate_limited_is_informational_success():
    ok, etype, msg, outcome = sunbiz_run_verdict(
    ...
    # outcome deliberately None — forcing SOURCE_ERROR here would flip
    # run_success to False via derive_run_success and regress this exact fix.
...
def test_rate_limited_wins_even_with_failures():
    ...
    ok, etype, _, outcome = sunbiz_run_verdict(
        {"processed": 10, "failed": 5, "rate_limited": True, "remaining_unprocessed": 190}
    )
    assert ok is True and etype == "rate_limited" and outcome is None


def test_empty_stats_does_not_divide_by_zero():
    assert sunbiz_run_verdict({}) == (True, None, None, None)
