...
from src.tasks.daily_dashboard import _fetch_lifecycle_autonomy, _fetch_lifecycle_decision_stats
...
def test_fetch_lifecycle_autonomy_uses_platform_denominator():
    session = _Session(_Result(fetchone=(100, 28)))

    assert _fetch_lifecycle_autonomy(session, date(2026, 6, 4)) == "28.0%"
...
def test_fetch_lifecycle_autonomy_returns_na_without_finalized_decisions():
    session = _Session(_Result(fetchone=(0, 0)))

    assert _fetch_lifecycle_autonomy(session, date(2026, 6, 4)) == "N/A"


def test_fetch_lifecycle_decision_stats_uses_same_autonomy_definition():
    session = _Session(_Result(fetchall=[
        ("fomo", 25, 12, 8, 5, 7, 1234, 0.0123),
    ]))

    rows = _fetch_lifecycle_decision_stats(session, date(2026, 6, 4))
