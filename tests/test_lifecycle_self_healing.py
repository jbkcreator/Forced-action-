"""
fa034 — Lifecycle self-healing tests.
...
class TestIncidentLifecycle:
    ...
    def test_new_incident_opens_when_metric_breaches(self):
        ...
        from src.tasks.lifecycle_self_healing import _process_metric, _Counters
        ...
        with patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=18), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=None), \
             patch("src.tasks.lifecycle_self_healing.post_incident_alert") as mock_post:
    ...
    def test_incident_stays_open_while_metric_still_bad(self):
        ...
        counters = _Counters(kill_recs_today=0)
        with patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=2), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=None), \
             patch("src.tasks.lifecycle_self_healing.post_incident_alert"):
            result = _process_metric(
    ...
    def test_incident_closes_when_metric_recovers(self):
        ...
        with patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=6), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=None), \
             patch("src.tasks.lifecycle_self_healing.post_incident_alert") as mock_post:
            result = _process_metric(
                session, metric_name="lock_conversion", county_id="hillsborough",
...
class TestActionTriggers:
    ...
    def test_48hr_breach_triggers_fallback_for_lock_conversion(self):
        ...
        with patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=2), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=None), \
             patch("src.tasks.lifecycle_self_healing._apply_fallback") as mock_apply, \
             patch("src.tasks.lifecycle_self_healing.post_incident_alert") as mock_post:
            result = _process_metric(
                session, metric_name="lock_conversion", county_id="hillsborough",
                feature_name=None, counters=_Counters(0), dry_run=False,
    ...
    def test_48hr_breach_escalates_for_first_payment(self):
        ...
        with patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=18), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=30), \
             patch("src.tasks.lifecycle_self_healing._apply_fallback") as mock_apply, \
             patch("src.tasks.lifecycle_self_healing._apply_ab_pause") as mock_ab, \
             patch("src.tasks.lifecycle_self_healing.post_incident_alert") as mock_post:
            result = _process_metric(
    ...
    def test_yellow_band_covers_sub_25pct_48h_escalation(self):
        ...
        with patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=24), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=30), \
             patch("src.tasks.lifecycle_self_healing._apply_fallback") as mock_apply, \
             patch("src.tasks.lifecycle_self_healing._apply_ab_pause") as mock_ab, \
             patch("src.tasks.lifecycle_self_healing.post_incident_alert") as mock_post:
...
class TestMasterSwitch:
    ...
    def test_master_kill_switch_disabled_is_noop(self):
        """15. LIFECYCLE_SELF_HEALING_ENABLED=false → return 0 without DB/Redis."""
        from src.tasks.lifecycle_self_healing import main

        with patch("src.tasks.lifecycle_self_healing.get_settings") as mock_settings:
            mock_settings.return_value = MagicMock(lifecycle_self_healing_enabled=False)
            with patch("src.tasks.lifecycle_self_healing.run_self_healing") as mock_run:
                exit_code = main(argv=[])
        assert exit_code == 0
...
class TestSlackEmailFallback:
    ...
    def test_slack_disabled_falls_back_to_email(self):
        """13. lifecycle_incident_slack_channel unset → send_alert is called."""
        ...
        with patch("src.services.lifecycle_slack.get_settings") as mock_settings:
            mock_settings.return_value = MagicMock(
                slack_bot_token=None,
                lifecycle_incident_slack_channel=None,
            )
...
class TestRevenuePulseIntegration:
    ...
    def test_revenue_pulse_includes_unresolved_incident(self):
        ...
        # _format_lifecycle_incident_alert is the helper; mock the session it sees.
        ...
        result = rp._format_lifecycle_incident_alert(fake_session)
    ...
    def test_weekly_summary_omits_when_no_activity(self):
        ...
        result = rp._format_lifecycle_incidents_weekly_summary(fake_session)
        assert result is None
...
class TestGraphKillSwitchWiring:
    """14. Verify each Lifecycle graph passes the correct kill_switch_feature value
...
class TestNoDuplicateIncidents:
    ...
    def test_no_duplicate_incidents_when_already_open(self):
        ...
        # No INSERT INTO lifecycle_incident appears in the SQL traces.
        for call in session.calls:
            assert "INSERT INTO lifecycle_incident" not in call["sql"]
...
class TestAbEngineReuse:
    ...
    def test_ab_engine_pause_path_reused_when_auto_paused(self):
        ...
        """
        from src.tasks import lifecycle_self_healing
        from src.tasks.lifecycle_self_healing import _process_metric, _Counters
        ...
        patched_ks = dict(lifecycle_self_healing.KILL_SWITCH)
        ...
        with patch.object(lifecycle_self_healing, "KILL_SWITCH", patched_ks), \
             patch("src.tasks.lifecycle_self_healing.get_cached_metric", return_value=2), \
             patch("src.tasks.lifecycle_self_healing.compute_baseline", return_value=None), \
...
# ---------------------------------------------------------------------------
# fa036 regression — kill recommendation writes a lifecycle_playbook row
# ---------------------------------------------------------------------------
...
class TestFa036KillWritesPlaybook:
    ...
    lifecycle_playbook row with status='recommended', authored_by='lifecycle', and
    ...
    def test_kill_path_calls_upsert_with_correct_attribution(self):
        ...
        with source_type='self_healing_kill', authored_by='lifecycle', and the
        ...
        assert kwargs["authored_by"] == "lifecycle"
