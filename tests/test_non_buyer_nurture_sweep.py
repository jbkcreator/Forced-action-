...
def _patched(campaign_id="camp_shared", candidates=None, enroll_result=None):
    settings = MagicMock()
...
def test_sweep_enrolls_found_candidates():
    candidates = [{"email": "a@example.com"}, {"email": "b@example.com"}]
    p_settings, p_dbctx, p_find, p_enroll, db = _patched(candidates=candidates, enroll_result={"enrolled": 2, "retried": 0})

    with p_settings, p_dbctx, p_find, p_enroll as mock_enroll:
...
def test_sweep_dry_run_does_not_enroll():
    ...
    p_settings, p_dbctx, p_find, p_enroll, db = _patched(candidates=candidates)
...
def test_sweep_skips_when_campaign_not_configured():
    p_settings, p_dbctx, p_find, p_enroll, db = _patched(campaign_id=None)

    with p_settings, p_dbctx, p_find as mock_find, p_enroll as mock_enroll:
        result = non_buyer_nurture_sweep.run()
