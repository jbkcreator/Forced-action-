...
def test_eligible_subscriber_produces_correctly_shaped_target(monkeypatch):
    sub = _subscriber()
    _patch_sourcing(monkeypatch, [sub])

    produced = win_back_producer.produce_win_back_targets(MagicMock())
...
def test_ineligible_subscriber_skipped(monkeypatch):
    sub = _subscriber()
...
def test_recently_attempted_by_live_reactivation_system_skipped(monkeypatch):
...
def test_attempted_outside_safety_window_is_not_skipped(monkeypatch):
...
def test_duplicate_actionable_draft_skipped(monkeypatch):
    sub = _subscriber()
    _patch_sourcing(monkeypatch, [sub])
