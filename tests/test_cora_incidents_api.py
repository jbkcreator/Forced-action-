"""
HTTP-level tests for the Cora self-healing incident management endpoints.

Covers:
  GET  /api/admin/cora-incidents
  GET  /api/admin/cora-incidents/{id}
  POST /api/admin/cora-incidents/{id}/acknowledge
  POST /api/admin/cora-incidents/{id}/resolve
"""
...
class TestAuthRequired:
    ...
    def test_list_requires_auth(self, client):
        r = client.get("/api/admin/cora-incidents")
        assert r.status_code in (401, 403)

    def test_detail_requires_auth(self, client):
        r = client.get("/api/admin/cora-incidents/1")
        assert r.status_code in (401, 403)

    def test_acknowledge_requires_auth(self, client):
        r = client.post("/api/admin/cora-incidents/1/acknowledge", json={})
        assert r.status_code in (401, 403)

    def test_resolve_requires_auth(self, client):
        r = client.post("/api/admin/cora-incidents/1/resolve", json={})
        assert r.status_code in (401, 403)
...
class TestListCoraIncidents:

    def test_returns_200_and_paginated_shape(self, client, auth):
        row = _incident_row()
        sess = _mock_session(all_rows=[row])
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.get("/api/admin/cora-incidents", headers=auth)
    ...
    def test_severity_filter_rejected_if_invalid(self, client, auth):
        r = client.get("/api/admin/cora-incidents?severity=critical", headers=auth)
        assert r.status_code == 422

    def test_action_taken_filter_rejected_if_invalid(self, client, auth):
        r = client.get("/api/admin/cora-incidents?action_taken=unknown_action", headers=auth)
        assert r.status_code == 422

    def test_inverted_date_range_rejected(self, client, auth):
        r = client.get(
            "/api/admin/cora-incidents"
    ...
    def test_open_only_filter_passes_through(self, client, auth):
        sess = _mock_session(all_rows=[])
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.get("/api/admin/cora-incidents?open_only=true", headers=auth)
        assert r.status_code == 200
    ...
    def test_severity_filter_included_in_sql(self, client, auth):
        sess = _mock_session(all_rows=[])
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.get("/api/admin/cora-incidents?severity=red", headers=auth)
        assert r.status_code == 200
...
# ---------------------------------------------------------------------------
# GET /api/admin/cora-incidents/{incident_id}
# ---------------------------------------------------------------------------

class TestGetCoraIncident:
    ...
    def test_returns_200_and_full_row(self, client, auth):
        ...
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.get("/api/admin/cora-incidents/1", headers=auth)
        assert r.status_code == 200
        assert r.json()["id"] == 1
    ...
    def test_404_when_not_found(self, client, auth):
        ...
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.get("/api/admin/cora-incidents/9999", headers=auth)
        assert r.status_code == 404
        assert "9999" in r.json()["detail"]


# ---------------------------------------------------------------------------
# POST /api/admin/cora-incidents/{incident_id}/acknowledge
# ---------------------------------------------------------------------------
...
class TestAcknowledgeIncident:
    ...
    def test_acknowledges_no_op_incident_and_upgrades_to_human_escalated(self, client, auth):
        ...
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.post(
                "/api/admin/cora-incidents/1/acknowledge",
                headers=auth,
    ...
    def test_404_when_incident_missing(self, client, auth):
        ...
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.post(
                "/api/admin/cora-incidents/9999/acknowledge",
                headers=auth,
                json={},
            )
        assert r.status_code == 404
...
# ---------------------------------------------------------------------------
# POST /api/admin/cora-incidents/{incident_id}/resolve
# ---------------------------------------------------------------------------
...
class TestResolveIncident:
    ...
    def test_resolves_open_incident(self, client, auth):
        ...
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.post(
                "/api/admin/cora-incidents/1/resolve",
                headers=auth,
    ...
    def test_404_when_incident_missing(self, client, auth):
        ...
        with patch("src.api.cora_incidents_router.get_db_context", _db_ctx(sess)):
            r = client.post(
                "/api/admin/cora-incidents/9999/resolve",
                headers=auth,
                json={},
            )
        assert r.status_code == 404
