...
Two things under test:
  1. _trigger_hot_inbound_callback (src.api.main) publishes inbound_hot_callback
     only when the inbound scored hot AND resolved to a subscriber.
...
class TestTriggerHotInboundCallback:
    def test_not_hot_does_not_publish(self):
        with patch("src.agents.events.ingestion.publish_cora_event") as mock_publish:
    ...
    def test_hot_with_subscriber_id_publishes_inbound_hot_callback(self):
        with patch("src.agents.events.ingestion.publish_cora_event") as mock_publish:
            _trigger_hot_inbound_callback(
        ...
        (event,), _kwargs = mock_publish.call_args
