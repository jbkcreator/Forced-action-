# Lifecycle Self-Diagnosis Reference

**Status:** Supervised-mode policy reference
**Scope:** The 7 monitored kill-switch metrics, their thresholds, and what Lifecycle is — and is not — allowed to do autonomously when each one breaches.

Source of truth (do not edit thresholds here; edit the code and regenerate this doc):
- Thresholds & action policy → `config/lifecycle_guardrails.py` (`KILL_SWITCH`, `LIFECYCLE_SELF_HEALING`)
- Stage 10 overrides → `config/stage10_config.py` (`STAGE10_KILL_SWITCH_OVERRIDES`)
- State machine & action dispatch → `src/tasks/lifecycle_self_healing.py`
...
- Lifecycle guardrail ranges → `config/lifecycle_guardrails.py` (`GUARDRAILS`)

Master switch: **`LIFECYCLE_SELF_HEALING_ENABLED`** (default `false`). When off, the loop is a no-op — it reads nothing and takes no action. Schedule: hourly cron (half-hour offset), after the metric-ingest job.

---

## 1. How a metric is graded

Every run, per metric, Lifecycle reads the **current value** (Redis cache, written daily by `kill_switch_metric_ingest`) and a **7-day rolling baseline** (`platform_daily_stats`), then grades severity against the metric's `green`/`red` thresholds:
...
- **Default policy:** `human_escalated`, `requires_approval=true`. Touches the checkout funnel + pricing — too sensitive for an autonomous fix. At 48h red, Lifecycle opens an incident, posts Slack, and surfaces it in Revenue Pulse; a human decides.
...
- **Policy:** `human_escalated`, `requires_approval=true`. **No autonomous action.** Auto-correct = none. At 48h red, escalates to a human (Slack + Revenue Pulse). The intended human play is "default harder / bonus credits" — a pricing/offer decision Lifecycle may not make alone.
...
- **Policy:** `fallback_enabled`, `requires_approval=false`. **Auto-correct:** at 48h red, set Redis `kill_switch:lifecycle_use_static_copy = red` (24h TTL). Lifecycle swaps from Claude-composed copy to the spec-approved static template (copy/timing issue is the usual cause). Reversible.
...
When a metric is **red continuously for `kill_after_red_days`** (7d most; 14d for `retention_30d`), Lifecycle records `action_taken=feature_killed` and writes a `lifecycle_playbook` recommendation + Slack "kill_recommended". **This is a recommendation only** — Lifecycle never disables a feature autonomously; a human approves the kill.
...
From `LIFECYCLE_SELF_HEALING` in `config/lifecycle_guardrails.py`:
...
All incident I/O is to `lifecycle_incident` via raw SQL. Every state transition posts to Slack (falls back to email if Slack unset — never a silent failure). A `--dry-run` mode reports intended actions without executing.

---

## 7. Supervised-mode quick policy

- **Lifecycle may act alone on exactly 4 metrics** (`wallet_adoption`, `lock_conversion`, `sms_reply_rate`, `offer_acceptance_rate`) — and only via a reversible 24h Redis fallback flag, after 48h in breach.
...
## 8. Lifecycle Guardrail Ranges (from FA-2B-v9-FINAL §3)

Lifecycle can optimise freely within these ranges. Anything outside requires Josh approval through Revenue Pulse.
...
These bounds are enforced at runtime by modules that consume `config.lifecycle_guardrails.GUARDRAILS`. The rollback triggers are monitored by the kill-switch automation; when a trigger fires, the feature auto-rolls back to its previous safe configuration.
...
- **Autonomy summary card:** Written at Monday 08:45 UTC by `lifecycle_autonomy_report.py` (15 minutes before Revenue Pulse)
...
| Incident activity (7d) | `lifecycle_incident` table | `2 red / 4 yellow open, 3 resolved, 1 kill-pending` |
| Autonomy summary | `learning_cards` (card_type='autonomy_summary') | `Lifecycle autonomy: 72% autonomous, 3% overridden, 4 adopted, +2 net playbooks` |
...
The scorecard is produced by `_format_kill_switch_scorecard()` in `src/tasks/revenue_pulse.py`. It reads each metric's current value from Redis (cached daily by `kill_switch_metric_ingest`), grades it using the same `_grade()` function that `lifecycle_self_healing` uses, and formats the compact line. If no metric data is available (Redis unavailable, or metric returns `None`), the metric is shown as `?` (unknown) — the same fail-safe no-op contract as the self-healing loop.
