# Cora Touch Timeline — Implementation Plan

Read-only UI surface that aggregates every Cora interaction for a single
subscriber. Backend in `Forced-action-`, frontend in `Forced-action-ui`.

## Glossary (already added to `CONTEXT.md`)

- **Cora touch** = one `agent_decisions` row. Composed SMS/voice/chat replies
  hang off the touch as child artifacts, not as separate touches.
- Pre-signup chat (no `agent_decisions` row) is **not** a Cora touch and is
  out of scope for v1.
...
| 4 | Backend mount | `src/api/cora_incidents_router.py` |
...
| 8 | Route | `/admin/cora-timeline/:subscriberId` (deep-link) |
...
### 2. New endpoint in `cora_incidents_router.py`

```
GET /api/admin/cora/subscribers/{subscriber_id}/timeline
...
```js
export async function fetchCoraTimeline(token, subscriberId, opts = {}) {
  ...
  return api.get(
    `/api/admin/cora/subscribers/${subscriberId}/timeline?${params}`,
    { token }
  );
}
```
...
- Register `/admin/cora-timeline/:subscriberId` as a child of `/admin` in
  `src/App.jsx` (uses existing `<Outlet />` shell).
- New page: `src/pages/admin/CoraTimelinePage.jsx`.
- Sidebar entry in `AdminSidebar`: "Cora Timeline" → prompts for subscriber
  ID, navigates to `/admin/cora-timeline/<id>` on submit.

### 6. Page structure (`CoraTimelinePage.jsx`)
...
- Each row component `<CoraTouchRow />`:
...
- Infinite scroll: bottom sentinel triggers `fetchCoraTimeline` with
  `cursor` from previous response. Stop when `next_cursor === null`.
- Loading: shimmer rows. Empty state: "No Cora touches for this subscriber."
...
- Add `/admin/cora-timeline/:subscriberId` to the Routes table.
