"""Auto-converted from alembic migration `fa036_lifecycle_autonomy_scorecard` (revision fa036_lifecycle_autonomy_scorecard).
...
Usage:
    PYTHONPATH=. python scripts/apply_fa036_lifecycle_autonomy_scorecard.py
"""
...
SQL = r"""
CREATE TABLE lifecycle_playbook (
    ...
    PRIMARY KEY (id), 
    CONSTRAINT check_lifecycle_playbook_status CHECK (status IN ('recommended','adopted','rejected','retired')), 
    CONSTRAINT fk_lifecycle_playbook_decision FOREIGN KEY(decision_id) REFERENCES agent_decisions (decision_id) ON DELETE SET NULL
);

CREATE INDEX idx_lifecycle_playbook_status ON lifecycle_playbook (status);

CREATE INDEX idx_lifecycle_playbook_authored ON lifecycle_playbook (authored_by, authored_at);

CREATE INDEX idx_lifecycle_playbook_adopted
            ON lifecycle_playbook(adopted_at)
            WHERE adopted_at IS NOT NULL;

CREATE UNIQUE INDEX idx_lifecycle_playbook_source_key_unique
            ON lifecycle_playbook(source_key)
...
ALTER TABLE agent_decisions ADD CONSTRAINT fk_agent_decisions_playbook FOREIGN KEY(playbook_id) REFERENCES lifecycle_playbook (id) ON DELETE SET NULL;
...
def main() -> None:
    ...
    logger.info("applied fa036_lifecycle_autonomy_scorecard")
