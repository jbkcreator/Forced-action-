"""Apply fa094_a6_lifecycle_training_overrides migration.

Usage:
    python scripts/apply_fa094_lifecycle_training_overrides.py
...
DDL = """
CREATE TABLE IF NOT EXISTS lifecycle_training_overrides (
    ...
    CONSTRAINT ck_lifecycle_overrides_source
        CHECK (source IN ('closer_teach', 'feedback_ritual')),
    CONSTRAINT ck_lifecycle_overrides_queue_status
        CHECK (queue_status IN ('pending', 'exported', 'discarded'))
);

CREATE INDEX IF NOT EXISTS idx_lifecycle_overrides_subject_active
    ON lifecycle_training_overrides (subject_type, subject_id, dampener_active);

CREATE INDEX IF NOT EXISTS idx_lifecycle_overrides_queue_status
    ON lifecycle_training_overrides (queue_status);
...
VALUES ('fa094_a6_lifecycle_training_overrides')
...
def main() -> None:
    ...
    with engine.begin() as conn:
        logger.info("Applying fa094: creating lifecycle_training_overrides table…")
        conn.execute(text(DDL))
        conn.execute(text(STAMP))
        logger.info("Done — lifecycle_training_overrides created and alembic_version stamped.")
