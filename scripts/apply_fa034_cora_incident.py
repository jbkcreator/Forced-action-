"""Auto-converted from alembic migration `fa034_cora_incident` (revision fa034_cora_incident).
...
Usage:
    PYTHONPATH=. python scripts/apply_fa034_cora_incident.py
"""
...
CREATE TABLE cora_incident (
    ...
    PRIMARY KEY (id), 
    CONSTRAINT check_cora_incident_severity CHECK (severity IN ('yellow','red')), 
    CONSTRAINT check_cora_incident_action CHECK (action_taken IN ('no_op','fallback_enabled','auto_paused','human_escalated','feature_killed','resolved'))
);
...
CREATE INDEX idx_cora_incident_breach_started ON cora_incident USING btree (breach_started);
...
def main() -> None:
    ...
    logger.info("applied fa034_cora_incident")
