"""
Lifecycle Guardrail Ranges — Phase 2B

Concrete bounds for every autonomous Lifecycle decision. Lifecycle can optimize freely
...
KILL_SWITCH = {
    "first_payment_rate":  {
        ...
        # in red Lifecycle opens an incident, posts Slack, surfaces in Revenue
    ...
    "lock_conversion":     {
        ...
        # Auto-fallback drops Lifecycle to static template SMS for the lock
    ...
    "retention_30d":       {
        ...
        # ingested), treat as green so Lifecycle can still run retention flows.
        "no_metric_behavior": "green",
    },
    "sms_reply_rate":      {
        ...
        "duration_hours_for_action": 48,
        "auto_action_type": "fallback_enabled",
        "fallback_feature_flag": "lifecycle_use_static_copy",
        "requires_approval": False,
        "kill_after_red_days": 7,
        "direction": "higher_is_better",
    },
...
# ── fa034 Lifecycle self-healing — rate limits and global knobs ──────────────────
...
LIFECYCLE_SELF_HEALING = {
    # Max number of automatic-actions taken in a single run (across all metrics).
    # Prevents a multi-metric breach from triggering a cascade of changes Lifecycle
