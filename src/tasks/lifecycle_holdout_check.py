...
frozen control by >2σ, both arms >=30 assignments) writes:
    ...
    1. A lifecycle_playbook recommendation (upsert_recommendation, idempotent by
    ...
    2. A LearningCard (card_type='holdout_result' — its own constrained
       ...
       'ab_result', which ab_rollback_check.py/lifecycle_attribution_rollback_
       ...
       future Lifecycle decisions see the result.
...
Run manually: python -m src.tasks.lifecycle_holdout_check [--dry-run]
...
def _write_learning_card(test_name: str, verdict: dict, db) -> None:
    ...
    UNIQUE table-wide — matches ab_rollback_check.py/lifecycle_attribution_
...
def run(dry_run: bool = False) -> dict:
    ...
    with get_db_context() as db:
        ...
        for test in holdout_tests:
            stats["checked"] += 1
            try:
                # conversion_window_days is registry-driven (config/lifecycle_holdout_
...
if __name__ == "__main__":
    ...
    logger.info("lifecycle_holdout_check result: %s", run(dry_run=dry))
