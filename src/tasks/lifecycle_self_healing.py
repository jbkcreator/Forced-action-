"""
Lifecycle self-healing loop (fa034).

Reads kill-switch metrics + 7-day baselines, runs a state machine over the
lifecycle_incident table, and dispatches safe corrective actions within
...
`config/lifecycle_guardrails.py`.
...
Master switch:
  LIFECYCLE_SELF_HEALING_ENABLED env var. Default `false`. When false, this
...
Rate limits (from LIFECYCLE_SELF_HEALING in lifecycle_guardrails):
...
Usage:
    python -m src.tasks.lifecycle_self_healing
    python -m src.tasks.lifecycle_self_healing --dry-run
"""
...
from config.lifecycle_guardrails import LIFECYCLE_SELF_HEALING, KILL_SWITCH, get_effective_kill_switch
...
def _open_incident(
    ...
    *,
    ...
    row = db.execute(sa_text("""
        INSERT INTO lifecycle_incident (
...
def _count_today_kill_recommendations(db: Session) -> int:
    row = db.execute(sa_text("""
        SELECT COUNT(*) AS c FROM lifecycle_incident
...
def _apply_fallback(feature_flag: str) -> None:
    ...
    24h TTL. The Lifecycle graph decision_hierarchy reads this on the next
    decision and routes to fallback template copy / disabled path.
    """
    if not redis_available():
        logger.warning("[lifecycle-self-heal] redis unavailable — cannot apply fallback %s", feature_flag)
        return
...
def _apply_variant_promotion(db: Session, metric_name: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.warning("[lifecycle-self-heal] Stage 10 variant_engine not importable")
        return None
...
def _process_metric(
    ...
    *,
    ...
    if open_incident is None:
        ...
        if counters.incidents_opened + _count_new_incidents_last_hour(db) >= LIFECYCLE_SELF_HEALING["max_new_incidents_per_hour"]:
    ...
    if severity == "red" and age_days >= kill_after_red_days:
        ...
        if counters.kill_recs_today >= LIFECYCLE_SELF_HEALING["max_feature_kill_recommendations_per_day"]:
            return {"metric": metric_name, "result": "skipped_rate_limit_kill"}
        if counters.actions_taken >= LIFECYCLE_SELF_HEALING["max_actions_per_run"]:
        ...
        # fa036 — write a `lifecycle_playbook` recommendation row so the kill
        # appears in Metric 5 ("net new playbooks Lifecycle authored") and is
        ...
        try:
            ...
            logger.warning(
                "[lifecycle-self-heal] playbook write failed for %s — incident still recorded",
...
def run_self_healing(dry_run: bool = False) -> dict:
    ...
    logger.info("[lifecycle-self-heal] %s", json.dumps(summary, default=str))
    return summary
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    Gated by `LIFECYCLE_SELF_HEALING_ENABLED` env var, default `false`. When
    ...
    if not settings.lifecycle_self_healing_enabled:
        logger.info(
            "[lifecycle-self-heal] disabled via LIFECYCLE_SELF_HEALING_ENABLED — exiting cleanly"
        )
        return 0
