"""
Lifecycle anomaly check — call volume + no-answer rate monitor.
...
calls in the last 60 minutes and pages the ops phone number via Twilio SMS if:
  - total calls exceed LIFECYCLE_VOLUME_THRESHOLD (default 40)
  - no-answer/voicemail rate exceeds LIFECYCLE_NO_ANSWER_THRESHOLD (default 0.85)
...
def run_anomaly_check(dry_run: bool = False) -> dict:
    ...
    logger.info(
        "[LifecycleMonitor] calls_60m=%d no_answer=%d rate=%.0f%% anomalies=%d",
        total, no_answer, no_answer_rate * 100, len(anomalies),
    )

    if anomalies:
        if dry_run:
            logger.info("[LifecycleMonitor] DRY RUN — would alert: %s", anomalies)
        else:
...
def _send_alert(settings, anomalies: list[str], total: int, no_answer: int) -> None:
    ...
    if not phone:
        logger.warning("[LifecycleMonitor] no alert phone configured (ALERT_SMS_NUMBER / FOUNDER_PHONE) — SMS skipped")
        return
    lines = ["[FA] Lifecycle anomaly alert"]
    for a in anomalies:
        lines.append(f"• {a}")
    ...
    try:
        with get_db_context() as db:
            sent = send_sms(
                ...
                task_type="lifecycle_anomaly",
                campaign="lifecycle_anomaly_alert",
            )
        if sent:
            logger.info("[LifecycleMonitor] alert SMS sent to %s", phone)
        else:
            logger.info("[LifecycleMonitor] alert SMS suppressed by compliance gate")
    except Exception as exc:
        logger.error("[LifecycleMonitor] alert SMS failed: %s", exc)
