"""
Lifecycle SMS outbound sender — DBPR contractor signup via SMS.

Targets enriched DBPR contacts that have a phone but haven't signed up yet.
For each contact:
  1. create_free_account(phone, source='lifecycle_sms') — deduped on phone
...
Run:
    python -m src.tasks.lifecycle_sms_sender --dry-run
    python -m src.tasks.lifecycle_sms_sender --limit 50
    python -m src.tasks.lifecycle_sms_sender --county-id pinellas --limit 100
"""
...
def run_lifecycle_sms_sender(
    ...
    if not candidates:
        logger.info("[LifecycleSmS] No eligible contacts (county=%s)", county_id)
        return stats

    stats["total"] = len(candidates)
    logger.info("[LifecycleSMS] %d contacts to SMS (county=%s, dry_run=%s)",
    ...
    for i, contact in enumerate(candidates):
        ...
        if dry_run:
            token = "DRY_RUN_TOKEN"
            url   = f"{settings.app_base_url}/?signup_source=lifecycle_sms&token={token}"
            body  = _sms_body(contact.full_name, vertical, url)
            logger.info("[LifecycleSMS DRY RUN] Would SMS %s | %s", phone, body[:80])
            stats["sent"] += 1
            continue
        ...
        with get_db_context() as db:
            sub = create_free_account(
                phone=phone,
                source="lifecycle_sms",
                db=db,
                county_id=county_id,
            )
            is_deduped = sub.signup_source != "lifecycle_sms"

        # Step 2 — signed link
        token = encode_landing_token(sub.id, "lifecycle_sms", ttl_hours=72)
        if token:
            landing_url = f"{settings.app_base_url}/?signup_source=lifecycle_sms&token={token}"
        else:
        ...
        with get_db_context() as db:
            ...
            ok = can_send(phone, db) and send_sms(
                ...
                task_type="lifecycle_sms_signup",
            )
        ...
        if ok:
            ...
            if is_deduped:
                stats["deduped"] += 1
                logger.info("[LifecycleSMS] Deduped (%d/%d) → sub=%d phone=%s",
                            i + 1, len(candidates), sub.id, phone)
            else:
                stats["sent"] += 1
                logger.info("[LifecycleSMS] Sent (%d/%d) → sub=%d phone=%s",
                            i + 1, len(candidates), sub.id, phone)
        else:
            stats["failed"] += 1
            logger.warning("[LifecycleSMS] Send failed → phone=%s", phone)

        if i < len(candidates) - 1:
            time.sleep(delay)

    logger.info(
        "[LifecycleSMS] Done. sent=%d deduped=%d failed=%d skipped=%d / total=%d",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lifecycle SMS outbound sender for DBPR contacts")
    ...
    try:
        result = run_lifecycle_sms_sender(
        ...
        logger.error("[LifecycleSMS] Sender crashed: %s", e)
        sys.exit(1)
