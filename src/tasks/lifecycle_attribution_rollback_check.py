"""
Lifecycle Attribution Rollout — auto-rollback monitor.

Runs every 120-180 seconds. Evaluates whether the 'variant' arm of the
lifecycle_attribution_v1 rollout test is losing vs 'control' by >2σ over the
rolling 48h window (requires ≥30 per arm). On trigger:
  ...
  3. Writes a learning card (card_type='ab_result') for future Lifecycle decisions.
...
Run manually: python -m src.tasks.lifecycle_attribution_rollback_check [--dry-run]
