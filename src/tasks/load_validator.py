...
# (HEARTBEAT_SLAS + SOURCE_OFF_DAYS). load_validator stays focused on quality
# checks against rows that *did* get written: failed runs, zero-record runs,
# and per-source volume drops. Keeping the two responsibilities separate
# prevents both watchdogs from emailing about the same incident.
...
def run_load_validator(county_id: str = "hillsborough") -> dict:
    ...
    with get_db_context() as session:
        ...
        # Missing-scraper detection has moved to heartbeat_monitor.
        # load_validator only evaluates rows that DID get written.
