"""
Weekly Lifecycle Autonomy Scorecard (fa036).
...
Metrics (per FA-2B-v9-FINAL §Weekly Lifecycle Autonomy Scorecard):

  1. autonomous_pct          — % decisions Lifecycle made without human approval
  ...
  3. recommended_adoptions   — count(lifecycle_playbook adopted in window)
  ...
  5. net_new_playbooks       — Lifecycle-authored playbooks in window - retired
                                playbooks in window.
...
Usage:
    python -m src.tasks.lifecycle_autonomy_report
    python -m src.tasks.lifecycle_autonomy_report --dry-run
"""
...
def _metric_3_adoptions(db: Session, since: datetime) -> int:
    ...
    return db.execute(sa_text("""
        SELECT COUNT(*) FROM lifecycle_playbook
...
def _metric_5_net_new_playbooks(db: Session, since: datetime) -> int:
    """Lifecycle-authored playbooks created in window MINUS retired in window."""
    row = db.execute(sa_text("""
        SELECT
            COUNT(*) FILTER (
                WHERE authored_by = 'lifecycle' AND authored_at >= :since
            ) AS authored,
            COUNT(*) FILTER (
...
def format_summary_text(m: dict) -> str:
    ...
    return "Lifecycle autonomy: " + ", ".join(parts)
...
def _post_slack_summary(metrics: dict, summary: str) -> None:
    """Post the autonomy summary to Slack via the existing lifecycle_slack
    ...
    try:
        ...
        fake_row = SimpleNamespace(
            metric_name="lifecycle_autonomy",
        ...
        logger.warning(
            "[lifecycle-autonomy] Slack post failed; metric still persisted",
            exc_info=True,
        )
...
def run_autonomy_report(dry_run: bool = False) -> dict:
    ...
    with get_db_context() as db:
        ...
        logger.info("[lifecycle-autonomy] %s", summary)
        logger.info("[lifecycle-autonomy] raw metrics: %s", json.dumps(metrics, default=str))
