...
def _do_transition(
    *,
    ...
    # Qualification owns its revision-keyed handoff. Other transitions use
    # the same durable builder queue, never an inline external effect.
    if source_component != "src.agents.fa_max.qualification_worker":
        _maybe_trigger_quote_ready_review(
            session=session, entity_type=entity_type, to_state=to_state,
...
def _maybe_trigger_quote_ready_review(
    *,
    ...
    """Enqueue builder work in the caller's transaction; never post Slack."""
    if entity_type != "opportunity" or to_state not in _QUOTE_READY_TRIGGER_STATES:
        return
    if not opportunity_id:
        return
    from src.services.quote_ready.workflow import enqueue_transition_build
    enqueue_transition_build(session, opportunity_id=opportunity_id)
