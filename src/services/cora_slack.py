"""
Slack helper for Cora self-healing incidents (fa034).

Posts incident notifications to the channel configured by
`CORA_INCIDENT_SLACK_CHANNEL` using the WebClient pattern already proven
...
attributes from cora_incident). `kind` is one of:
    - "new"            — incident just opened, before any action
    - "action_taken"   — Cora took an automatic action within guardrails
...
_KIND_PRELUDE = {
    "new":              ":warning: Cora opened a new incident",
    "action_taken":     ":robot_face: Cora took an automatic action",
    "human_required":   ":rotating_light: Cora needs human review",
    "kill_recommended": ":no_entry: Cora recommends killing a feature",
    "resolved":         ":white_check_mark: Cora incident resolved",
}
...
def post_incident_alert(
    ...
    """Post a Cora incident notification.
    ...
    if token and channel:
        try:
            ...
            logger.warning("[cora_slack] slack_sdk not installed — falling back to email")
        else:
            try:
                ...
                logger.warning(
                    "[cora_slack] Slack post failed (%s) — falling back to email",
                ...
                logger.warning("[cora_slack] Slack post raised — falling back to email", exc_info=True)
    ...
    try:
        ...
        logger.warning("[cora_slack] email fallback raised — no notification sent", exc_info=True)
        return None
