"""
Subscriber-level suppression for Cora-led outbound messages.

This module intentionally does not handle TCPA STOP compliance. STOP remains in
sms_compliance; these helpers pause Cora follow-up after human engagement or
deal outcomes.
"""
...
from src.core.models import CoraSuppression, MessageOutcome, SmsOptIn

logger = logging.getLogger(__name__)

PENDING_CORA_STATUSES = ("pending_review", "approved")


def has_active_suppression(db: Session, subscriber_id: int) -> bool:
    """Return True when Cora outbound touches are paused for this subscriber."""
    if subscriber_id is None:
        return False
    row = db.execute(
        select(CoraSuppression.id).where(
            CoraSuppression.subscriber_id == subscriber_id,
            CoraSuppression.is_active.is_(True),
        ).limit(1)
    ).first()
...
def cancel_pending_messages(
    ...
    *,
    ...
    """Cancel queued Cora MessageOutcome rows for a subscriber."""
    now = now or datetime.now(timezone.utc)
    rows = db.execute(
        select(MessageOutcome).where(
            MessageOutcome.subscriber_id == subscriber_id,
            MessageOutcome.send_status.in_(PENDING_CORA_STATUSES),
    ...
    if rows:
        logger.info(
            "Cora suppression cancelled %d pending messages for subscriber=%s reason=%s",
            len(rows),
            subscriber_id,
            reason,
        )
...
def create_suppression(
    ...
    *,
    ...
    """Create an active suppression row and cancel queued Cora messages."""
    now = now or datetime.now(timezone.utc)
    suppression = CoraSuppression(
    ...
    db.flush()
    logger.info(
        "Cora suppression created subscriber=%s reason=%s source=%s source_id=%s",
        subscriber_id,
        reason,
        source,
        source_id,
...
def record_generic_sms_reply(
    ...
    *,
    ...
    now: Optional[datetime] = None,
) -> Optional[CoraSuppression]:
    """
    Mark the latest Cora SMS as replied and pause future Cora touches.
